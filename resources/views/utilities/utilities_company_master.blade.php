	...
	<div class="div-utilities">
		...
		<div class="div-title">
			<a href="{{ url('/utilities') }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('utilities_company_master.list') }}</span>
			</a>
		</div>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    var table = $('#company_master_table').DataTable({
        ...
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('utilities/company/table') }}",
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('utilities/company/detail_data') }}", { 'companyCode' : null, 'companyName' : null, 'npwpNo' : null, 'holdingCompany' : null, 'locationCode' : null, 'address' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('utilities/company/detail_data') }}", { 'companyCode' : data[0].companyCode, 'companyName' : data[0].companyName, 'npwpNo' : data[0].npwpNo, 'holdingCompany' : data[0].holdingCompany, 'locationCode' : data[0].locationCode, 'address' : data[0].address, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('utilities/company/status') }}",
                type: "GET",
    ...
    $('#company_master_table tbody').on('click', 'tr td:not(:first-child)', function () {
        ...
        $.redirect("{{ url('utilities/company/detail_data') }}", { 'companyCode' : data.companyCode, 'companyName' : data.companyName, 'npwpNo' : data.npwpNo, 'holdingCompany' : data.holdingCompany, 'locationCode' : data.locationCode, 'address' : data.address, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
