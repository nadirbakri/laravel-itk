...
<body>
	<div class="div-utilities">
		...
		<div class="div-table">
            <table id="master_banner_table" class="table hover">
                <thead>
                    <tr>
                        <th></th>
                        <th>Image</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('#master_banner_table thead tr').clone(true).appendTo('#master_banner_table thead');
    $('#master_banner_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#master_banner_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('utilities/master_banner/table') }}",
    ...
    function load_data_master_banner() {
        table = $('#master_banner_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('utilities/master_banner/table') }}",
    ...
    $('#master_banner_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#master_banner_table').on('click', 'tr td:first-child', function(e){
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('utilities/master_banner/detail_data') }}", { 'bannerID' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('utilities/master_banner/detail_data') }}", { 'bannerID' : data[0].bannerID, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $('#master_banner_table tbody').on('click', 'tr td:not(:first-child)', function () {
        var data = table.row(this).data();
        $.redirect("{{ url('utilities/master_banner/detail_data') }}", { 'bannerID' : data.bannerID, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
