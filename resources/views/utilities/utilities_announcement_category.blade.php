...
<html>
<head>
	<title>{{ __('utilities_announcement_category.judul') }}</title>
	...
	<div class="div-utilities">
		...
		<div class="div-title">
			<a href="{{ route('utilities', ['moduleID' => 'UTI']) }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('utilities_announcement_category.list') }}</span>
			</a>
		</div>
		<div class="div-table">
            <table id="announcement_category_table" class="table hover">
                <thead>
                    <tr>
                        <th></th>
                        <th>Category Code</th>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="div-title-notification">
                        ...
                        <span class="title-text-notification">{{ __('utilities_announcement_category.alert_success') }}</span>
                    </div>
                    <div class="div-title-notification">
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('#announcement_category_table thead tr').clone(true).appendTo('#announcement_category_table thead');
    $('#announcement_category_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#announcement_category_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('utilities/announcement_category/table') }}",
    ...
    function load_data_announcement_category() {
        table = $('#announcement_category_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('utilities/announcement_category/table') }}",
    ...
    $('#announcement_category_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#announcement_category_table').on('click', 'tr td:first-child', function(e){
        $(this).parent().find('input[type="checkbox"]').trigger('click');
    });

    $('#notification_success').on('hide.bs.modal', function () {
        window.location = "{{ url('utilities/announcement_category') }}";
    })

    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('utilities/announcement_category/detail_data') }}", { 'categoryCode' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('utilities/announcement_category/detail_data') }}", { 'categoryCode' : data[0].categoryCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('utilities/announcement_category/status') }}",
                type: "GET",
                data: { 'categoryCode' : data[0].categoryCode, 'categoryName' : data[0].categoryName, 'func' : 'A' },
                success: function(response) {
                    ...
                    var oTable = $('#announcement_category_table').dataTable();
                    oTable.fnDraw(false);
                },
    ...
    $("#toolbar-deactive").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('utilities/announcement_category/status') }}",
                type: "GET",
                data: { 'categoryCode' : data[0].categoryCode, 'categoryName' : data[0].categoryName, 'func' : 'D' },
                success: function(response) {
                    if(response.status == "true"){
                        $('#notification_success').modal('show');
                        $('#message-notification-success').html(response.message);
    ...
    $("#toolbar-remove").on('click', function() {
        ...
        if(data.count() > 0){
            if (confirm("Are you sure you want to delete this category?")) {
                $.ajax({
                    url: "{{ url('utilities/announcement_category/delete') }}",
                    type: "GET",
    ...
    $('#announcement_category_table tbody').on('click', 'tr td:not(:first-child)', function () {
        var data = table.row(this).data();
        $.redirect("{{ url('utilities/announcement_category/detail_data') }}", { 'categoryCode' : data.categoryCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
