	...
	<div class="div-medical">
		...
		<div class="div-title">
			<a href="{{ route('medical', ['moduleID' => 'MD']) }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('md_treatment_eligibility.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="treatment_eligibility_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Position Code</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#treatment_eligibility_table thead tr').clone(true).appendTo('#treatment_eligibility_table thead');
    $('#treatment_eligibility_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#treatment_eligibility_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('medical/treatment_eligibility/table') }}",
    ...
    function load_data_treatment_eligibility() {
        table = $('#treatment_eligibility_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('medical/treatment_eligibility/table') }}",
    ...
    $('#treatment_eligibility_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#treatment_eligibility_table').on('click', 'tr td:first-child', function(e){
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('medical/treatment_eligibility/detail_data') }}", { 'positionCode' : null, 'rankingCode' : null, 'serviceMonth' : null, 'claimCode' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('medical/treatment_eligibility/detail_data') }}", { 'positionCode' : data[0].positionCode, 'rankingCode' : data[0].rankingCode, 'serviceMonth' : data[0].serviceMonth, 'claimCode' : data[0].claimCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('medical/treatment_eligibility/status') }}",
                type: "GET",
                data: { 'positionCode' : data[0].positionCode, 'rankingCode' : data[0].rankingCode, 'serviceMonth' : data[0].serviceMonth, 'claimCode' : data[0].claimCode, 'func' : 'A' },
                success: function(response) {
                    ...
                    var oTable = $('#treatment_eligibility_table').dataTable();
                    oTable.fnDraw(false);
                },
    ...
    $("#toolbar-deactive").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('medical/treatment_eligibility/status') }}",
                type: "GET",
                data: { 'positionCode' : data[0].positionCode, 'rankingCode' : data[0].rankingCode, 'serviceMonth' : data[0].serviceMonth, 'claimCode' : data[0].claimCode, 'func' : 'D' },
                success: function(response) {
                    if(response.status == "true"){
                        $('#notification_success').modal('show');
                        $('#message-notification-success').html(response.message);
    ...
    $('#treatment_eligibility_table tbody').on('click', 'tr td:not(:first-child)', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('medical/treatment_eligibility/detail_data') }}", { 'positionCode' : data.positionCode, 'rankingCode' : data.rankingCode, 'serviceMonth' : data.serviceMonth, 'claimCode' : data.claimCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
