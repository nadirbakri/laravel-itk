...
<body>
	<div class="div-medical">
		...
		<div class="div-table">
			<table id="input_personnel_limit_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Employee No</th>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#input_personnel_limit_table thead tr').clone(true).appendTo('#input_personnel_limit_table thead');
    $('#input_personnel_limit_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#input_personnel_limit_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('medical/input_personnel_limit/table') }}",
    ...
    function load_data_input_personnel_limit() {
        table = $('#input_personnel_limit_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('medical/input_personnel_limit/table') }}",
    ...
    $('#input_personnel_limit_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#input_personnel_limit_table').on('click', 'tr td:first-child', function(e){
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('medical/input_personnel_limit/detail_data') }}", { 'employeeNo' : null, 'claimCode' : null, 'expiredDate' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('medical/input_personnel_limit/detail_data') }}", { 'employeeNo' : data[0].employeeNo, 'claimCode' : data[0].claimCode, 'expiredDate' : data[0].expiredDate, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('medical/input_personnel_limit/status') }}",
                type: "GET",
                data: { 'employeeNo' : data[0].employeeNo, 'claimCode' : data[0].claimCode, 'expiredDate' : data[0].expiredDate, 'func' : 'A' },
                success: function(response) {
                    ...
                    var oTable = $('#input_personnel_limit_table').dataTable();
                    oTable.fnDraw(false);
                },
    ...
    $("#toolbar-deactive").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('medical/input_personnel_limit/status') }}",
                type: "GET",
                data: { 'employeeNo' : data[0].employeeNo, 'claimCode' : data[0].claimCode, 'expiredDate' : data[0].expiredDate, 'func' : 'D' },
                success: function(response) {
                    if(response.status == "true"){
                        $('#notification_success').modal('show');
                        $('#message-notification-success').html(response.message);
    ...
    $('#input_personnel_limit_table tbody').on('click', 'tr td:not(:first-child)', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('medical/input_personnel_limit/detail_data') }}", { 'employeeNo' : data.employeeNo, 'claimCode' : data.claimCode, 'expiredDate' : data.expiredDate, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
