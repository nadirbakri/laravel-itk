...
<html>
<head>
	<title>{{ __('medical_insurance_class.judul') }}</title>
...
<body>
	<div class="div-medical">
		...
		<div class="div-table">
			<table id="insurance_class_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Insurance Class Code</th>
						<th>Insurance Class Description</th>
                        <th>Record Status</th>
					</tr>
				</thead>
			</table>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="div-title-notification">
                        ...
                        <span class="title-text-notification">{{ __('medical_insurance_class.alert_success') }}</span>
                    </div>
                    <div class="div-title-notification">
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#insurance_class_table thead tr').clone(true).appendTo('#insurance_class_table thead');
    $('#insurance_class_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#insurance_class_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('medical/insurance_class/table') }}",
        ...
        columns: [
            ...
            {data: 'insuranceClassDescription', name: 'insuranceClassDescription'},
    ...
    function load_data_insurance_code() {
        table = $('#insurance_class_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('medical/insurance_class/table') }}",
    ...
    $('#insurance_class_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#insurance_class_table').on('click', 'tr td:first-child', function(e){
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('medical/insurance_class/detail_data') }}", { 'insuranceClassCode' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('medical/disease_code/detail_data') }}", { 'insuranceClassCode' : data[0].insuranceClassCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('medical/insurance_class/status') }}",
                type: "GET",
                data: { 'insuranceClassCode' : data[0].insuranceClassCode, 'func' : 'A' },
                success: function(response) {
                    ...
                    var oTable = $('#insurance_class_table').dataTable();
                    oTable.fnDraw(false);
                },
    ...
    $("#toolbar-deactive").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('medical/insurance_class/status') }}",
                type: "GET",
                data: { 'insuranceClassCode' : data[0].insuranceClassCode, 'func' : 'D' },
                success: function(response) {
                    if(response.status == "true"){
                        $('#notification_success').modal('show');
                        $('#message-notification-success').html(response.message);
    ...
    $('#insurance_class_table tbody').on('click', 'tr td:not(:first-child)', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('medical/insurance_class/detail_data') }}", { 'insuranceClassCode' : data.insuranceClassCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
