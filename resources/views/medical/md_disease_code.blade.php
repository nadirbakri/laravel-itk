...
<body>
	<div class="div-medical">
		...
		<div class="div-table">
			<table id="disease_code_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Disease Code</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#disease_code_table thead tr').clone(true).appendTo('#disease_code_table thead');
    $('#disease_code_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#disease_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('medical/disease_code/table') }}",
        ...
        columns: [
            ...
            {data: 'diseaseCode', name: 'diseaseCode'},
    ...
    function load_data_disease_code() {
        table = $('#disease_code_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('medical/disease_code/table') }}",
    ...
    $('#disease_code_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#disease_code_table').on('click', 'tr td:first-child', function(e){
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('medical/disease_code/detail_data') }}", { 'diseaseCode' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('medical/disease_code/detail_data') }}", { 'diseaseCode' : data[0].diseaseCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('medical/disease_code/status') }}",
                type: "GET",
                data: { 'diseaseCode' : data[0].diseaseCode, 'func' : 'A' },
                success: function(response) {
                    ...
                    var oTable = $('#disease_code_table').dataTable();
                    oTable.fnDraw(false);
                },
    ...
    $("#toolbar-deactive").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('medical/disease_code/status') }}",
                type: "GET",
                data: { 'diseaseCode' : data[0].diseaseCode, 'func' : 'D' },
                success: function(response) {
                    if(response.status == "true"){
                        $('#notification_success').modal('show');
                        $('#message-notification-success').html(response.message);
    ...
    $('#disease_code_table tbody').on('click', 'tr td:not(:first-child)', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('medical/disease_code/detail_data') }}", { 'diseaseCode' : data.diseaseCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
