...
<body>
	<div class="div-medical">
		...
		<div class="div-table">
			<table id="claim_transaction_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Employee No</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#claim_transaction_table thead tr').clone(true).appendTo('#claim_transaction_table thead');
    $('#claim_transaction_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#claim_transaction_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('medical/claim_transaction/table') }}",
    ...
    function load_data_claim_transaction() {
        table = $('#claim_transaction_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('medical/claim_transaction/table') }}",
    ...
    $('#claim_transaction_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#claim_transaction_table').on('click', 'tr td:first-child', function(e){
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('medical/claim_transaction/detail_data') }}", { 'employeeNo' : null, 'claimDate' : null, 'seqNo' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('medical/claim_transaction/detail_data') }}", { 'employeeNo' : data[0].employeeNo, 'claimDate' : data[0].claimDate, 'seqNo' : data[0].seqNo, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $('#claim_transaction_table tbody').on('click', 'tr td:not(:first-child)', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('medical/claim_transaction/detail_data') }}", { 'employeeNo' : data.employeeNo, 'claimDate' : data.claimDate, 'seqNo' : data.seqNo, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
