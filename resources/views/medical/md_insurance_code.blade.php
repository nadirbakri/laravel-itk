...
<html>
<head>
	<title>{{ __('medical_insurance_code.judul') }}</title>
	...
	<div class="div-medical">
		...
		<div class="div-title">
			<a href="javascript:void(0);" onclick="goBackWithModuleID()" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('medical_insurance_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="insurance_code_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="div-title-notification">
                        ...
                        <span class="title-text-notification">{{ __('medical_insurance_code.alert_success') }}</span>
                    </div>
                    <div class="div-title-notification">
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#insurance_code_table thead tr').clone(true).appendTo('#insurance_code_table thead');
    $('#insurance_code_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#insurance_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('medical/insurance_code/table') }}",
        ...
        columns: [
            ...
            {data: 'insuranceDescription', name: 'insuranceDescription'},
    ...
    function load_data_insurance_code() {
        table = $('#insurance_code_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('medical/insurance_code/table') }}",
    ...
    $('#insurance_code_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#insurance_code_table').on('click', 'tr td:first-child', function(e){
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('medical/insurance_code/detail_data') }}", { 'insuranceCode' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('medical/insurance_code/detail_data') }}", { 'insuranceCode' : data[0].insuranceCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('medical/insurance_code/status') }}",
                type: "GET",
                data: { 'insuranceCode' : data[0].insuranceCode, 'func' : 'A' },
                success: function(response) {
                    ...
                    var oTable = $('#insurance_code_table').dataTable();
                    oTable.fnDraw(false);
                },
    ...
    $("#toolbar-deactive").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('medical/insurance_code/status') }}",
                type: "GET",
                data: { 'insuranceCode' : data[0].insuranceCode, 'func' : 'D' },
                success: function(response) {
                    if(response.status == "true"){
                        $('#notification_success').modal('show');
                        $('#message-notification-success').html(response.message);
    ...
    $('#insurance_code_table tbody').on('click', 'tr td:not(:first-child)', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('medical/insurance_code/detail_data') }}", { 'insuranceCode' : data.insuranceCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
