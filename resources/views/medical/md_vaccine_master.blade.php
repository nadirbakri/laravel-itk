...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#toolbar-new").on('click', function() {
            $.redirect("{{ url('medical/vaccine_master/detail_data') }}", { 'batch' : null, 'code' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
        });

        $("#toolbar-edit").on('click', function() {
            var data = table.rows('.selected').data();
            if(data.count() > 0){
                $.redirect("{{ url('medical/vaccine_master/detail_data') }}", { 'batch' : data[0].batch, 'code' : data[0].code, 'func' : 'edit' }, "GET", "iframe_dashboard");
            }else{
        ...
        $('#vaccine_master_table tbody').on('click', 'tr td:not(:first-child)', function () {
            ...
            $.redirect("{{ url('medical/vaccine_master/detail_data') }}", { 'batch' : data.batch, 'code' : data.code, 'func' : 'edit' }, "GET", "iframe_dashboard");
        });
