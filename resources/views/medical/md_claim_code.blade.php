...
<html>
<head>
	<title>{{ __('medical_claim_code.judul') }}</title>
	...
	<div class="div-medical">
		...
		<div class="div-title">
			<a href="{{ url('medical') }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('medical_claim_code.list') }}</span>
			</a>
		</div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="div-title-notification">
                        ...
                        <span class="title-text-notification">{{ __('medical_claim_code.alert_success') }}</span>
                    </div>
                    <div class="div-title-notification">
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    var table = $('#claim_code_table').DataTable({
        ...
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('medical/claim_code/table') }}",
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('medical/claim_code/detail_data') }}", { 'claimCode' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('medical/claim_code/detail_data') }}", { 'claimCode' : data[0].claimCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('medical/claim_code/status') }}",
                type: "GET",
    ...
    $('#claim_code_table tbody').on('click', 'tr td:not(:first-child)', function () {
    	...
    	$.redirect("{{ url('medical/claim_code/detail_data') }}", { 'claimCode' : data.claimCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
