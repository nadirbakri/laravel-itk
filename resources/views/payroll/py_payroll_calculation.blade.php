    ...
    $(document).ready(function () {
        ...
        function load_data_table_payroll_calculation() {
            table = $('#payroll_calculation_table').DataTable({
                ...
                serverSide: true,
                orderCellsTop: true,
                ajax: "{{ url('payroll/payroll_calculation/table') }}",
        ...
        $("#toolbar-new").on('click', function() {
            $.redirect("{{ url('payroll/payroll_calculation/detail') }}", { 'fieldName' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
        });

        $("#toolbar-edit").on('click', function() {
            var data = table.rows('.selected').data();
            if(data.count() > 0){
                $.redirect("{{ url('payroll/payroll_calculation/detail') }}", { 'fieldName' : data[0].fieldName, 'func' : 'edit' }, "GET", "iframe_dashboard");
            }else{
