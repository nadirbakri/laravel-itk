...
<html>
    ...
    <body>
        ...
        <h2 style="text-align:center; font-weight:bold;">LOAN SUMMARY REPORT</h2>
        <p style="text-align: center">Month : {{date('F Y', strtotime($period))}}</p>
        <table style="width: 100%; border-collapse: collapse; border-spacing: 0;" class="table table-bordered table-hover responsive table_detail">
            <thead>
                <tr>
                    ...
                    <th rowspan="2" style="border-top: 1px solid black; border-bottom: 1px solid black; padding:4px; font-weight: bold; text-align: center; vertical-align: middle;">Amount</th>
                    <th rowspan="2" style="border-top: 1px solid black; border-bottom: 1px solid black; padding:4px; font-weight: bold; text-align: center; vertical-align: middle;">Installment</th>
                </tr>
                <tr>
                ...
                @foreach($data as $key => $value)
                    @foreach($value->loanEmployeeList as $key2 => $value2)
                        <tr>
                            ...
                            <td data-format="#,##0.00" style="text-align:center; padding:4px;">{{number_format($value2->loanAmount, 2, '.', ',')}}</td>
                            <td data-format="#,##0.00" style="text-align:center; padding:4px;">{{number_format($value2->installmentPerMonth, 2, '.', ',')}}</td>
                        </tr>
                    @endforeach
                @endforeach
                <tr>
                    <td style="border-bottom: 1px solid black; font-weight:bold; text-align:left; padding:4px;" colspan="5">&nbsp;</td>
                </tr>
                @if(isset($value))
                    <tr>
                        <td colspan="3" style="text-align:left; padding:4px; font-weight:bold;">Grand Total</td>
                        <td data-format="#,##0.00" style="font-weight:bold; text-align:center; padding:4px;">{{number_format($value->grandTotalLoan, 2, '.', ',')}}</td>
                        <td data-format="#,##0.00" style="font-weight:bold; text-align:center; padding:4px;">{{number_format($value->grandTotalInstallment, 2, '.', ',')}}</td>
                    </tr>
                @endif
            </tbody>
        </table>
