                            ...
                            @if(!empty($dataTable->data[0]->field))
                                @foreach($dataTable->data[0]->field as $key_data => $dataRow)
                                    @if($loop->first)
                                        ...
                                        @if($company == 'IPN' || $company == 'UPM' || $company == 'IGT' || $company == 'IVT' || $company == 'IPNJT')
                                        <th colspan="2" style="text-align:center; vertical-align:middle; border:1px solid #000; padding:4px; background-color: #97d7f7; font-size: {{ $dataRow->fontSize }}px !important; font-weight: bold;">{{ $group_name ?? 'Cost Center' }}</th>
                                        @else
                                        <th style="text-align:center; vertical-align:middle; border:1px solid #000; padding:4px; background-color: #97d7f7; font-size: {{ $dataRow->fontSize }}px !important; font-weight: bold;">{{ $group_name ?? 'Cost Center' }}</th>
                                        @endif
                    ...
                    <table style="width: 100%; marginY: 2" class="table table-bordered table-hover responsive table_detail">
                        ...
                        <tr>
                            ...
                            @foreach($total[$dataTable->data[0]->companyName] as $key => $totalValue)
                                <td data-format="#,##0" style="text-align:right; border:1px solid #000;">{{ number_format($totalValue, 0, '.', ',') }}</td>
                            @endforeach
                        </tr>
                    </table>
                @endif
