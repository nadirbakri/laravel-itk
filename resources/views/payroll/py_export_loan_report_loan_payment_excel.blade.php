    ...
    <table style="width: 100%;" class="table table-bordered table-hover responsive table_detail">
        ...
        @if(count($data) > 0)
            <tbody>
                @foreach($data[0]->loanReportData as $key => $value)
                    ...
                    @foreach($value->listData as $key2 => $value2)
                        <tr>
                            ...
                            <td style="border:1px solid #000; text-align:center; padding:4px;">{{date('d M Y', strtotime($value2->paymentDate))}}</td>
                ...
                @endforeach
                <tr>
                    <td colspan="7" style="border:1px solid #000; text-align:left; padding:4px; font-weight:bold;">Grand Total</td>
