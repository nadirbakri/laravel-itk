...
<html>
<head>
    <title>{{ __('export_salary_complaint.judul') }}</title>
...
<body>
    <div class="div-form">
        <form id="export_salary_complaint_form" method="post">
            @csrf
            <div class="div-export-salary-complaint">
                <div class="div-title">
                    <a href="{{ route('export', ['moduleID' => 'RPT']) }}" target="iframe_dashboard">
                        <img src="{{ url('/pictures/arrow-square-left.png') }}" alt="Back">
                        <span class="title-text">{{ __('export_salary_complaint.list') }}</span>
                    </a>
                </div>
                ...
                <div class="row">
                    <div class="col-3">
                        <div class="form-group">
                            <label for="claim_date_from form-check-label">{{ __('export_salary_complaint.label_claim_date') }}</label>
                        </div>
                    </div>
                    <div class="col-4">
                        <div class="input-group">
                            <input type="text" class="form-control" id="claim_date_from" name="claim_date_from"
                                placeholder="{{ __('export_salary_complaint.label_claim_start') }}">
                    ...
                    <div class="col-0.5">
                        <div class="form-group">
                            <label for="claim_date_to form-check-label">{{ __('export_salary_complaint.label_to') }}</label>
                        </div>
                    </div>
                    <div class="col-4">
                        <div class="input-group">
                            <input type="text" class="form-control" id="claim_date_to" name="claim_date_to"
                                placeholder="{{ __('export_salary_complaint.label_claim_end') }}">
                            <div class="input-group-prepend">
                ...
                <div class="row">
                    <div class="col-3">
                        <div class="form-group">
                            <label for="business_unit">{{ __('export_salary_complaint.label_business_unit') }}</label>
                        </div>
                    </div>
                    <div class="col-4">
                        <div class="form-group">
                ...
                <div class="row">
                    <div class="col-3">
                        <div class="form-group">
                            <label for="salary_complaint_status">{{ __('export_salary_complaint.label_salary_complaint_status') }}</label>
                        </div>
                    </div>
                    <div class="col-4">
                        <div class="form-group">
                            <select class="form-control select2" id="salary_complaint_status" name="salary_complaint_status"></select>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-3">
                        <button class="btn btn-primary" name="btn-preview" id="btn-preview" value="preview" style="width: 100%;">
                            <i class="fa fa-download"></i> {{ __('export_salary_complaint.btn_export') }}
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="div-title-notification">
                        ...
                        <span class="title-text-notification">{{ __('export_salary_complaint.alert_success') }}</span>
                    </div>
                    <div class="div-title-notification">
...
loadDataFirstLastAllStatus();
    ...
    //     });
        ...
        function loadDataStatus(){
            ...
            $('#salary_complaint_status').select2({
                width: '100%',
        ...
        function loadDataFirstLastAllStatus() {
            $('#salary_complaint_status').addClass('spinner-border');

            $.ajax({
                type: 'GET',
                url: "{{ url('/status_trans/api') }}",
            }).then(function (data) {
                $('#salary_complaint_status').prepend($('<option>').val('ALL').text('ALL'));
                $('#salary_complaint_status option:contains("ALL")').not(':first').remove();
                $('#salary_complaint_status').val('ALL');
                $('#salary_complaint_status').removeClass('spinner-border');
            });
        }
        ...
        $("#btn-preview").click(function () {
            ...
            $("#export_salary_complaint_form").submit();
        });

        if ($("#export_salary_complaint_form").length > 0) {
            $("#export_salary_complaint_form").validate({
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        ...
                        url: "{{ url('export/export_salary_complaint/print') }}",
                        type: "POST",
                        data: $('#export_salary_complaint_form').serialize(),
