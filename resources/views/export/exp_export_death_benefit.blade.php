...
<html>
<head>
    <title>{{ __('export_death_benefit.judul') }}</title>
...
<body>
    <div class="div-form">
        <form id="export_death_benefit_form" method="post">
            @csrf
            <div class="div-export-death-benefit">
                <div class="div-title">
                    <a href="{{ route('export', ['moduleID' => 'RPT']) }}" target="iframe_dashboard">
                        <img src="{{ url('/pictures/arrow-square-left.png') }}" alt="Back">
                        <span class="title-text">{{ __('export_death_benefit.list') }}</span>
                    </a>
                </div>
                ...
                <div class="row">
                    <div class="col-3">
                        <div class="form-group">
                            <label for="claim_date_from form-check-label">{{ __('export_death_benefit.label_claim_date') }}</label>
                        </div>
                    </div>
                    <div class="col-4">
                        <div class="input-group">
                            <input type="text" class="form-control" id="claim_date_from" name="claim_date_from"
                                placeholder="{{ __('export_death_benefit.label_claim_start') }}">
                    ...
                    <div class="col-0.5">
                        <div class="form-group">
                            <label for="claim_date_to form-check-label">{{ __('export_death_benefit.label_to') }}</label>
                        </div>
                    </div>
                    <div class="col-4">
                        <div class="input-group">
                            <input type="text" class="form-control" id="claim_date_to" name="claim_date_to"
                                placeholder="{{ __('export_death_benefit.label_claim_end') }}">
                            <div class="input-group-prepend">
                ...
                <div class="row">
                    <div class="col-3">
                        <div class="form-group">
                            <label for="business_unit">{{ __('export_death_benefit.label_business_unit') }}</label>
                        </div>
                    </div>
                    <div class="col-4">
                        <div class="form-group">
                ...
                <div class="row">
                    <div class="col-3">
                        <div class="form-group">
                            <label for="death_benefit_status">{{ __('export_death_benefit.label_death_benefit_status') }}</label>
                        </div>
                    </div>
                    <div class="col-4">
                        <div class="form-group">
                            <select class="form-control select2" id="death_benefit_status" name="death_benefit_status"></select>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-3">
                        <button class="btn btn-primary" name="btn-preview" id="btn-preview" value="preview" style="width: 100%;">
                            <i class="fa fa-download"></i> {{ __('export_death_benefit.btn_export') }}
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="div-title-notification">
                        ...
                        <span class="title-text-notification">{{ __('export_death_benefit.alert_success') }}</span>
                    </div>
                    <div class="div-title-notification">
...
loadDataFirstLastAllStatus();
    ...
    //     });
        ...
        function loadDataFirstLastAllStatus() {
            $('#death_benefit_status').addClass('spinner-border');

            $.ajax({
                type: 'GET',
                url: "{{ url('/status_trans/api') }}",
            }).then(function (data) {
                $('#death_benefit_status').prepend($('<option>').val('ALL').text('ALL'));
                $('#death_benefit_status option:contains("ALL")').not(':first').remove();
                $('#death_benefit_status').val('ALL');
                $('#death_benefit_status').removeClass('spinner-border');
            });
        }
        ...
        $("#btn-preview").click(function () {
            ...
            $("#export_death_benefit_form").submit();
        });

        if ($("#export_death_benefit_form").length > 0) {
            $("#export_death_benefit_form").validate({
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        ...
                        url: "{{ url('export/export_death_benefit/print') }}",
                        type: "POST",
                        data: $('#export_death_benefit_form').serialize(),
