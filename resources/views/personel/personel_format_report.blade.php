...
<html>
<head>
	<title>{{ __('personel_format_report.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ route('personnel', ['moduleID' => 'PE']) }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_format_report.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="format_report_table" class="table hover">
				<thead>
					<tr>
						<th>Report Code</th>
						<th>Report Name</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
  	$('#format_report_table thead tr').clone(true).appendTo('#format_report_table thead');
    $('#format_report_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#format_report_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personnel/format_report/table') }}",
    ...
    $('#format_report_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personnel/format_report/detail_data') }}", { 'report_code' : data.report_code }, "GET", "iframe_dashboard");
    });
