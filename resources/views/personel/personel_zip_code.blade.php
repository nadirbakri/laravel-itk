	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ route('personnel', ['moduleID' => 'PE']) }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_zip_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="zip_code_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Zip Code</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#zip_code_table thead tr').clone(true).appendTo('#zip_code_table thead');
    $('#zip_code_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#zip_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
    ...
    function load_data_zip_code() {
        table = $('#zip_code_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('personnel/zip/table') }}",
        ...
        if(data.count() > 0){
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var oTable = $('#zip_code_table').dataTable();
                    oTable.fnDraw(false);
                },
