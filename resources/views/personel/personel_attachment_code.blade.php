...
<html>
<head>
	<title>{{ __('personel_attachment_code.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="javascript:void(0);" onclick="goBackWithModuleID()" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_attachment_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="attachment_code_table" class="table hover">
				<thead>
					<tr>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
  	$('#attachment_code_table thead tr').clone(true).appendTo('#attachment_code_table thead');
    $('#attachment_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#attachment_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personnel/attachment_code/table') }}",
    ...
    $('#attachment_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personnel/attachment_code/detail_data') }}", { 'attachment_code' : data.attachment_code }, "GET", "iframe_dashboard");
    });
