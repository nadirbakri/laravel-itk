	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ url('personel') }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_bank_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="bank_code_table" class="table hover">
				<thead>
					<tr>
						<th>Bank Code</th>
						<th>Bank Name</th>
...
<script type="text/javascript">
  $(document).ready(function() {
  	$('#bank_code_table thead tr').clone(true).appendTo('#bank_code_table thead');
    $('#bank_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#bank_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personel/bank_code/table') }}",
        ...
        columns: [
            ...
            {data: 'number_of_digits', name: 'number_of_digits'},
            {data: 'record_status', name: 'record_status'}
        ]
    });

    $('#bank_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personel/bank_code/detail_data') }}", { 'bank_code' : data.bank_code }, "GET", "iframe_dashboard");
    });
