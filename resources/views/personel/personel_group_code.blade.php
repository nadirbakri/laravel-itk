	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ url('personel') }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_group_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="group_code_table" class="table hover">
				<thead>
					<tr>
						<th>Group Code</th>
						<th>Group Name</th>
...
<script type="text/javascript">
  $(document).ready(function() {
  	$('#group_code_table thead tr').clone(true).appendTo('#group_code_table thead');
    $('#group_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#group_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personel/group_code/table') }}",
    ...
    $('#group_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personel/group_code/detail_data') }}", { 'group_code' : data.group_code }, "GET", "iframe_dashboard");
    });
