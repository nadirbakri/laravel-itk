	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="javascript:void(0);" onclick="goBackWithModuleID()" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_nature_of_work.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="nature_of_work_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#nature_of_work_table thead tr').clone(true).appendTo('#nature_of_work_table thead');
    $('#nature_of_work_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#nature_of_work_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: {
        ...
        columns: [
            ...
            {data: 'workNatureCode', name: 'workNatureCode'},
            {data: 'workNatureName', name: 'workNatureName'},
    ...
    function load_data_nature_of_work() {
        table = $('#nature_of_work_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
    ...
    $('#nature_of_work_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#nature_of_work_table').on('click', 'tr td:first-child', function(e){
        ...
        if(data.count() > 0){
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var oTable = $('#nature_of_work_table').dataTable();
                    oTable.fnDraw(false);
                },
        ...
        if(data.count() > 0){
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // $('#nature_of_work_table').DataTable().destroy();
                    // $('#nature_of_work_table').empty();
                    // load_data_nature_of_work();
                    var oTable = $('#nature_of_work_table').dataTable();
                    oTable.fnDraw(false);
                },
                error: function(response) {
