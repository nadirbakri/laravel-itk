...
<html>
<head>
	<title>{{ __('personel_npwp_group.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="javascript:void(0);" onclick="goBackWithModuleID()" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_npwp_group.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="npwp_group_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>NPWP Code</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#npwp_group_table thead tr').clone(true).appendTo('#npwp_group_table thead');
    $('#npwp_group_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#npwp_group_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ...
        columns: [
            ...
            {data: 'companyNPWP', name: 'companyNPWP'},
    ...
    function load_data_npwp_group() {
        table = $('#npwp_group_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('personnel/npwp_group/table') }}",
    ...
    $('#npwp_group_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#npwp_group_table').on('click', 'tr td:first-child', function(e){
        ...
        if(data.count() > 0){
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var oTable = $('#npwp_group_table').dataTable();
                    oTable.fnDraw(false);
                },
