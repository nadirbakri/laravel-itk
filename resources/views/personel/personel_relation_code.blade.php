...
<html>
<head>
	<title>{{ __('personel_relation_code.judul') }}</title>
       ...
       <div class="div-navbar sticky-navbar">
		...
		<div class="div-title">
			<a href="{{ route('personnel', ['moduleID' => 'PE']) }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_relation_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="relation_code_table" class="table hover">
				<thead>
					<tr>
						<th>Relation Code</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
  	$('#relation_code_table thead tr').clone(true).appendTo('#relation_code_table thead');
    $('#relation_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#relation_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ...
        columns: [
            {data: 'relation_code', name: 'relation_code'},
            {data: 'relation_description', name: 'relation_description'},
