...
<html>
<head>
	<title>{{ __('personel_education_status_code.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="javascript:void(0);" onclick="goBackWithModuleID()" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_education_status_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="education_status_code_table" class="table hover">
				<thead>
					<tr>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
  	$('#education_status_code_table thead tr').clone(true).appendTo('#education_status_code_table thead');
    $('#education_status_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#education_status_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
