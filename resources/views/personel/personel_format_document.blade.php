...
<html>
<head>
	<title>{{ __('personel_format_document.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ url('personel') }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_format_document.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="format_document_table" class="table hover">
				<thead>
					<tr>
						<th>Document Code</th>
						<th>Document Name</th>
...
<script type="text/javascript">
  $(document).ready(function() {
  	$('#format_document_table thead tr').clone(true).appendTo('#format_document_table thead');
    $('#format_document_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#format_document_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
