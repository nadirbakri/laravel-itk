...
<html>
<head>
	<title>{{ __('personel_plafon_medical.judul_tunjangan') }}</title>
    ...
    <div class="div-navbar sticky-navbar">
		...
		<div class="div-title">
			<a href="{{ route('personnel', ['moduleID' => 'PE']) }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_plafon_medical.list_detail_tunjangan') }}</span>
			</a>
		</div>
...
<script type="text/javascript">
    ...
    function handleFileUpload(input) {
        ...
        if (file) {
            if (!allowedExtensions.exec(file.name)) {
                ...
                formData.append('type', 'TUNJANGAN');
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response[0].status == "true") {
                            ...
                            // setTimeout(function () {
                            //     window.location =
                            //         "{{ url('personnel/plafon_tunjangan') }}";
                            // }, 3000);
                        } else {
                            $("#toolbar-import").prop("disabled", false);
                            $("#toolbar-import").html(
  ...
  $(document).ready(function() {
    ...
    var table = $('#plafon_table').DataTable({
        ...
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personnel/plafon_tunjangan/table') }}",
    ...
    $('#notification_success').on('hide.bs.modal', function () {
        window.location = "{{ url('personnel/plafon_tunjangan') }}";
    })
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('personnel/plafon_tunjangan/detail_data') }}", { 'plafonYear' : null, 'plafonCode' : null, 'rankCode' : null, 'category' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('personnel/plafon_tunjangan/detail_data') }}", { 'plafonYear' : data[0].plafonYear, 'plafonCode' : data[0].plafonCode, 'rankCode' : data[0].rankCode, 'category' : data[0].category, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
            $('#notification_error').modal('show');
    ...
    $('#plafon_table tbody').on('click', 'tr td:not(:first-child)', function () {
    	...
    	$.redirect("{{ url('personnel/plafon_tunjangan/detail_data') }}", { 'plafonYear' : data.plafonYear, 'plafonCode' : data.plafonCode, 'rankCode' : data.rankCode, 'category' : data.category, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
