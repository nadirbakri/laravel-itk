...
<html>
<head>
	<title>{{ __('personel_termination_code.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ url('/personnel') }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_termination_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="termination_code_table" class="table hover">
				<thead>
					<tr>
...
<script type="text/javascript">
  $(document).ready(function() {
  	$('#termination_code_table thead tr').clone(true).appendTo('#termination_code_table thead');
    $('#termination_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#termination_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personnel/termination_code/table') }}",
    ...
    $('#termination_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personnel/termination_code/detail_data') }}", { 'termination_code' : data.termination_code }, "GET", "iframe_dashboard");
    });
