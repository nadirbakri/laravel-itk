...
<html>
<head>
	<title>{{ __('personel_insurance_code.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ url()->previous() }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_insurance_code.list') }}</span>
			</a>
		</div>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('#insurance_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#insurance_code_table').DataTable({
        ...
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personnel/insurance_code/table') }}",
        ...
        columns: [
            ...
            {data: 'insurance_description', name: 'insurance_description'},
            {data: 'record_status', name: 'record_status'}
        ]
    });

    $('#insurance_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personnel/insurance_code/detail_data') }}", { 'insurance_code' : data.insurance_code }, "GET", "iframe_dashboard");
    });
