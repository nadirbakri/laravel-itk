...
<html>
<head>
	<title>{{ __('personel_loan_bank.judul') }}</title>
    ...
    <div class="div-navbar sticky-navbar">
		...
		<div class="div-title">
			<a href="{{ route('personnel', ['moduleID' => 'PE']) }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_loan_bank.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="loan_bank_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
                        <th>Loan Bank</th>
                        <th>Loan Company Code</th>
                        <th>Description</th>
					</tr>
				</thead>
			</table>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#loan_bank_table thead tr').clone(true).appendTo('#loan_bank_table thead');
    $('#loan_bank_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#loan_bank_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: {
            url: "{{ url('personnel/loan_bank/table') }}",
            cache: false
        },
        ...
        columns: [
            ...
            {data: 'loanBank', name: 'loanBank'},
            {data: 'loanCompanyCode', name: 'loanCompanyCode'},
    ...
    function load_data_loan_bank() {
        table = $('#loan_bank_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: {
    ...
    $('#loan_bank_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#loan_bank_table').on('click', 'tr td:first-child', function(e){
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('personnel/loan_bank/detail_data') }}", { 'loanBank' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('personnel/loan_bank/detail_data') }}", { 'loanBank' : data[0].loanBank, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $('#loan_bank_table tbody').on('click', 'tr td:not(:first-child)', function () {
        var data = table.row(this).data();
        $.redirect("{{ url('personnel/loan_bank/detail_data') }}", { 'loanBank' : data.loanBank, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
