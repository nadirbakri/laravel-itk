...
<html>
<head>
	<title>{{ __('personel_nationality_code.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="javascript:void(0);" onclick="goBackWithModuleID()" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_nationality_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="nationality_code_table" class="table hover">
				<thead>
					<tr>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
  	$('#nationality_code_table thead tr').clone(true).appendTo('#nationality_code_table thead');
    $('#nationality_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#nationality_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personnel/nationality_code/table') }}",
    ...
    $('#nationality_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personnel/nationality_code/detail_data') }}", { 'nationality_code' : data.nationality_code }, "GET", "iframe_dashboard");
    });
