	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="javascript:void(0);" onclick="goBackWithModuleID()" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_source_document.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="source_document_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Letter Type</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
  	$('#source_document_table thead tr').clone(true).appendTo('#source_document_table thead');
    $('#source_document_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#source_document_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ...
        columns: [
            ...
            {data: 'letterType', name: 'letterType'},
            {data: 'letterFile', name: 'letterFile', render: function ( data, type, row ) {
                return '<a href="' + '../letter_template/' + data + '" target="_blank">Preview</a>';
            } },
    ...
    function load_data_source_document() {
        table = $('#source_document_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('personnel/source_document/table') }}",
            ...
            columns: [
                ...
                {data: 'letterType', name: 'letterType'},
                {data: 'letterFile', name: 'letterFile'},
                {data: 'recordStatus', name: 'recordStatus', render: function ( data, type, row ) {
                    return '<a href="' + '../letter_template/' + data + '" target="_blank">Preview</a>';
                } }
            ],
            select: {
    ...
    $('#source_document_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#source_document_table').on('click', 'tr td:first-child', function(e){
        ...
        if(data.count() > 0){
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var oTable = $('#source_document_table').dataTable();
                    oTable.fnDraw(false);
                },
