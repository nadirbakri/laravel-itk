...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('#source_document_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#source_document_table').DataTable({
        ...
        columns: [
            {data: 'source_document', name: 'source_document'},
            {data: 'source_document_description', name: 'source_document_description'},
            {data: 'sequence_type', name: 'sequence_type'},
            {data: 'status', name: 'status'}
        ]
    });

    $('#source_document_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personel/source_document/detail_data') }}", { 'source_document' : data.source_document }, "GET", "iframe_dashboard");
    });
