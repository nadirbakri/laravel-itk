...
<html>
<head>
	<title>{{ __('personel_language_ability_code.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="javascript:void(0);" onclick="goBackWithModuleID()" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_language_ability_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="language_ability_code_table" class="table hover">
				<thead>
					<tr>
						<th>Language Ability Code</th>
						<th>Language Ability Descrption</th>
                        <th>Status</th>
					</tr>
				</thead>
			</table>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
  	$('#language_ability_code_table thead tr').clone(true).appendTo('#language_ability_code_table thead');
    $('#language_ability_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#language_ability_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personnel/language_ability_code/table') }}",
        ...
        columns: [
            {data: 'language_ability_code', name: 'language_ability_code'},
            {data: 'language_ability_description', name: 'language_ability_description'},
            {data: 'status', name: 'status'}
        ]
    });

    $('#language_ability_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personnel/language_ability_code/detail_data') }}", { 'language_ability_code' : data.language_ability_code }, "GET", "iframe_dashboard");
    });
