	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="javascript:void(0);" onclick="goBackWithModuleID()" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_free_format_field.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="free_format_field_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Free Format Code</th>
						<th>Field Type</th>
                        <th>Description</th>
                        <th>Length</th>
                        <th>Record Status</th>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="div-title-notification">
                        ...
                        <span class="title-text-notification">{{ __('personel_free_format_field.alert_success') }}</span>
                    </div>
                    <div class="div-title-notification">
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
  	$('#free_format_field_table thead tr').clone(true).appendTo('#free_format_field_table thead');
    $('#free_format_field_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#free_format_field_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ...
        columns: [
            ...
            {data: 'freeFormatCode', name: 'freeFormatCode'},
            {data: 'freeFormatFieldType', name: 'freeFormatFieldType'},
    ...
    function load_data_free_format_field() {
        table = $('#free_format_field_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('personnel/free_format_field/table') }}",
    ...
    $('#free_format_field_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#free_format_field_table').on('click', 'tr td:first-child', function(e){
        ...
        if(data.count() > 0){
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var oTable = $('#evaluation_form_table').dataTable();
                    oTable.fnDraw(false);
                },
