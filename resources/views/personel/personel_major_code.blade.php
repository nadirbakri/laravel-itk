...
<body>
	<div class="div-personel">
		...
		<div class="div-table">
			<table id="major_code_table" class="table hover">
				<thead>
					<tr>
						<th>Major Code</th>
						<th>Major Name</th>
...
<script type="text/javascript">
  $(document).ready(function() {
  	$('#major_code_table thead tr').clone(true).appendTo('#major_code_table thead');
    $('#major_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#major_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personel/major_code/table') }}",
    ...
    $('#major_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personel/major_code/detail_data') }}", { 'major_code' : data.major_code }, "GET", "iframe_dashboard");
    });
