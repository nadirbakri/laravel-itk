	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ route('personnel', ['moduleID' => 'PE']) }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_major_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="major_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Major Code</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
  	$('#major_table thead tr').clone(true).appendTo('#major_table thead');
    $('#major_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#major_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ...
        columns: [
            ...
            {data: 'majorCode', name: 'majorCode'},
            {data: 'majorName', name: 'majorName'},
    ...
    function load_data_major() {
        table = $('#major_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('personnel/major/table') }}",
    ...
    $('#major_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#major_table').on('click', 'tr td:first-child', function(e){
        ...
        if(data.count() > 0){
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var oTable = $('#major_table').dataTable();
                    oTable.fnDraw(false);
                },
