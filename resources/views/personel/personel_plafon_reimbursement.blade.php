    ...
    <div class="div-navbar sticky-navbar">
		...
		<div class="div-title">
			<a href="{{ route('personnel', ['moduleID' => 'PE']) }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_plafon_reimbursement.list') }}</span>
			</a>
		</div>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    var table = $('#plafon_table').DataTable({
        ...
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personnel/plafon_reimbursement/table') }}",
        ...
        columns: [
            ...
            {data: 'category', name: 'category',
                render: function (data, type, row) {
                    return data === 'RMB' ? 'REIMBURSEMENT' : data
                }
            },
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('personnel/plafon_reimbursement/detail_data') }}", { 'plafonYear' : null, 'plafonCode' : null, 'rankCode' : null, 'category' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('personnel/plafon_reimbursement/detail_data') }}", { 'plafonYear' : data[0].plafonYear, 'plafonCode' : data[0].plafonCode, 'rankCode' : data[0].rankCode, 'category' : data[0].category, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
            $('#notification_error').modal('show');
    ...
    $('#plafon_table tbody').on('click', 'tr td:not(:first-child)', function () {
    	...
    	$.redirect("{{ url('personnel/plafon_reimbursement/detail_data') }}", { 'plafonYear' : data.plafonYear, 'plafonCode' : data.plafonCode, 'rankCode' : data.rankCode, 'category' : data.category, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
