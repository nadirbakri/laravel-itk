...
<html>
<head>
	<title>{{ __('personel_language_code.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ route('personnel', ['moduleID' => 'PE']) }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_language_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="language_code_table" class="table hover">
				<thead>
					<tr>
						<th>Language Code</th>
						<th>Language Name</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
  	$('#language_code_table thead tr').clone(true).appendTo('#language_code_table thead');
    $('#language_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#language_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personnel/language_code/table') }}",
    ...
    $('#language_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personnel/language_code/detail_data') }}", { 'language_code' : data.language_code }, "GET", "iframe_dashboard");
    });
