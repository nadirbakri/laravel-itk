	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ route('personnel', ['moduleID' => 'PE']) }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_office_location_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="office_location_code_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#office_location_code_table thead tr').clone(true).appendTo('#office_location_code_table thead');
    $('#office_location_code_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#office_location_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: {
        ...
        columns: [
            ...
            {data: 'officeCode', name: 'officeCode', width: "15%" },
            {data: 'officeDesc', name: 'officeDesc', width: "20%"},
            {data: 'longitude', name: 'longitude'},
            {data: 'latitude', name: 'latitude'},
            {data: 'maxTolerance', name: 'maxTolerance'},
            {data: 'isLock', name: 'isLock'},
    ...
    function load_data_office_location_code() {
        table = $('#office_location_code_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: {
    ...
    $('#office_location_code_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#office_location_code_table').on('click', 'tr td:first-child', function(e){
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('personnel/office_location/detail_data') }}", { 'officeCode' : null, 'officeDesc' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('personnel/office_location/detail_data') }}", { 'officeCode' : data[0].officeCode, 'officeDesc' : data[0].officeDesc, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('personnel/office_location/status') }}",
                type: "GET",
                data: { 'officeCode' : data[0].officeCode, 'officeDesc' : data[0].officeDesc, 'func' : 'A' },
                success: function(response) {
                    ...
                    var oTable = $('#office_location_code_table').dataTable();
                    oTable.fnDraw(false);
                },
    ...
    $("#toolbar-deactive").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('personnel/office_location/status') }}",
                type: "GET",
                data: { 'officeCode' : data[0].officeCode, 'officeDesc' : data[0].officeDesc, 'func' : 'D' },
                success: function(response) {
                    if(response.status == "true"){
                        $('#notification_success').modal('show');
                        $('#message-notification-success').html(response.message);
    ...
    $('#office_location_code_table tbody').on('click', 'tr td:not(:first-child)', function () {
        var data = table.row(this).data();
        $.redirect("{{ url('personnel/office_location/detail_data') }}", { 'officeCode' : data.officeCode, 'officeDesc' : data.officeDesc, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
