    ...
    <div class="div-navbar sticky-navbar">
		...
		<div class="div-title">
			<a href="{{ route('personnel', ['moduleID' => 'PE']) }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_plafon_business_trip.list') }}</span>
			</a>
		</div>
...
<script type="text/javascript">
    ...
    function handleFileUpload(input) {
        ...
        if (file) {
            if (!allowedExtensions.exec(file.name)) {
                ...
                formData.append('type', 'BUSINESSTRIP');
  ...
  $(document).ready(function() {
    ...
    var table = $('#plafon_table').DataTable({
        ...
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personnel/plafon_business_trip/table') }}",
        ...
        columns: [
            ...
            {data: 'isDuty', name: 'isDuty',
                render: function (data, type, row) {
                    return data ? 'Kedinasan' : 'Non Kedinasan'
                }
            },
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('personnel/plafon_business_trip/detail_data') }}", { 'plafonYear' : null, 'plafonCode' : null, 'rankCode' : null, 'category' : null, 'isDuty' : null, 'status' : null, 'plafonAmount' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('personnel/plafon_business_trip/detail_data') }}", { 'plafonYear' : data[0].plafonYear, 'plafonCode' : data[0].plafonCode, 'rankCode' : data[0].rankCode, 'category' : data[0].category, 'isDuty' : data[0].isDuty, 'status' : data[0].status, 'plafonAmount' : data[0].plafonAmount, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
            $('#notification_error').modal('show');
    ...
    $('#plafon_table tbody').on('click', 'tr td:not(:first-child)', function () {
        ...
        // console.log(data)
    	$.redirect("{{ url('personnel/plafon_business_trip/detail_data') }}", { 'plafonYear' : data.plafonYear, 'plafonCode' : data.plafonCode, 'rankCode' : data.rankCode, 'category' : data.category, 'isDuty' : data.isDuty, 'status' : data.status, 'plafonAmount' : data.plafonAmount, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
