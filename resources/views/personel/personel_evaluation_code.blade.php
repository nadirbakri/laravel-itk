...
<script type="text/javascript">
  $(document).ready(function() {
  	$('#evaluation_code_table thead tr').clone(true).appendTo('#evaluation_code_table thead');
    $('#evaluation_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#evaluation_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personel/evaluation_code/table') }}",
    ...
    $('#evaluation_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personel/evaluation_code/detail_data') }}", { 'form_code' : data.form_code }, "GET", "iframe_dashboard");
    });
