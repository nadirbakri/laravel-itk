...
<body>
	<div class="div-personel">
		...
		<div class="div-table">
			<table id="title_code_table" class="table hover">
				<thead>
					<tr>
						<th>Title Code</th>
						<th>Description</th>
...
<script type="text/javascript">
  $(document).ready(function() {
  	$('#title_code_table thead tr').clone(true).appendTo('#title_code_table thead');
    $('#title_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#title_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personel/title_code/table') }}",
        ...
        columns: [
            {data: 'title_code', name: 'title_code'},
            {data: 'description', name: 'description'}
        ]
    });

    $('#title_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personel/title_code/detail_data') }}", { 'title_code' : data.title_code }, "GET", "iframe_dashboard");
    });
