    ...
    <div class="div-navbar sticky-navbar">
		...
		<div class="div-title">
			<a href="javascript:void(0);" onclick="goBackWithModuleID()" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_title_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="title_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Title Code</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('#title_table thead tr').clone(true).appendTo('#title_table thead');
    $('#title_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#title_table').DataTable({
        ...
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personnel/title/table') }}",
    ...
    function load_data_title() {
        table = $('#title_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('personnel/title/table') }}",
    ...
    $('#title_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#title_table').on('click', 'tr td:first-child', function(e){
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("{{ url('personnel/title/detail_data') }}", { 'titleCode' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("{{ url('personnel/title/detail_data') }}", { 'titleCode' : data[0].titleCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('personnel/title/status') }}",
                type: "GET",
                data: { 'titleCode' : data[0].titleCode, 'titleName' : data[0].titleName, 'func' : 'A' },
                success: function(response) {
                    ...
                    var oTable = $('#title_table').dataTable();
                    oTable.fnDraw(false);
                },
    ...
    $("#toolbar-deactive").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "{{ url('personnel/title/status') }}",
                type: "GET",
                data: { 'titleCode' : data[0].titleCode, 'titleName' : data[0].titleName, 'func' : 'D' },
                success: function(response) {
                    if(response.status == "true"){
                        $('#notification_success').modal('show');
                        $('#message-notification-success').html(response.message);
    ...
    $('#title_table tbody').on('click', 'tr td:not(:first-child)', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personnel/title/detail_data') }}", { 'titleCode' : data.titleCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
