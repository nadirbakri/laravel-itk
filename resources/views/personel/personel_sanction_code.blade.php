...
<html>
<head>
	<title>{{ __('personel_sanction_code.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ route('personnel', ['moduleID' => 'PE']) }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_sanction_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="sanction_code_table" class="table hover">
				<thead>
					<tr>
						<th>Sanction Code</th>
						<th>Sanction Name</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
  	$('#sanction_code_table thead tr').clone(true).appendTo('#sanction_code_table thead');
    $('#sanction_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#sanction_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personnel/sanction_code/table') }}",
        ...
        columns: [
            ...
            {data: 'sanction_name', name: 'sanction_name'},
            {data: 'status', name: 'status'}
        ]
    });

    $('#sanction_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personnel/sanction_code/detail_data') }}", { 'sanction_code' : data.sanction_code }, "GET", "iframe_dashboard");
    });
