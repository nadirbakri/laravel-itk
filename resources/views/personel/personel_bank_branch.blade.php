...
<html>
<head>
	<title>{{ __('personel_bank_branch.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ url('personel') }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_bank_branch.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="bank_branch_table" class="table hover">
				<thead>
					<tr>
						<th>Bank Code</th>
						<th>Bank Name</th>
...
<script type="text/javascript">
  $(document).ready(function() {
  	$('#bank_branch_table thead tr').clone(true).appendTo('#bank_branch_table thead');
    $('#bank_branch_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#bank_branch_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personel/bank_branch/table') }}",
        ...
        columns: [
            ...
            {data: 'bi_code', name: 'bi_code'},
            {data: 'status', name: 'status'}
        ]
    });

    $('#bank_branch_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personel/bank_branch/detail_data') }}", { 'bank_code' : data.bank_code }, "GET", "iframe_dashboard");
    });
