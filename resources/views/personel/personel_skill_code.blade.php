	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ route('personnel', ['moduleID' => 'PE']) }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_skill_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="skill_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Skill Code</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
  	$('#skill_table thead tr').clone(true).appendTo('#skill_table thead');
    $('#skill_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    function load_data_skill() {
        table = $('#skill_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ url('personnel/skill/table') }}",
    ...
    $('#skill_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#skill_table').on('click', 'tr td:first-child', function(e){
        ...
        if(data.count() > 0){
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var oTable = $('#skill_table').dataTable();
                    oTable.fnDraw(false);
                },
