...
<html>
<head>
	<title>{{ __('personel_education_code.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ url('/personnel') }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_education_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="education_code_table" class="table hover">
				<thead>
					<tr>
...
<script type="text/javascript">
  $(document).ready(function() {
  	$('#education_code_table thead tr').clone(true).appendTo('#education_code_table thead');
    $('#education_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#education_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personnel/education_code/table') }}",
        ...
        columns: [
            {data: 'education_code', name: 'education_code'},
            {data: 'education_name', name: 'education_name'},
            {data: 'status', name: 'status'}
        ]
    });

    $('#education_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personnel/education_code/detail_data') }}", { 'education_code' : data.education_code }, "GET", "iframe_dashboard");
    });
