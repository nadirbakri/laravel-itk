...
<html>
<head>
	<title>{{ __('personel_other_information.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ url('/personel') }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_other_information.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="other_information_table" class="table hover">
				<thead>
					<tr>
						<th>Employee No</th>
						<th>Employee Name</th>
...
<script type="text/javascript">
  $(document).ready(function() {
  	$('#other_information_table thead tr').clone(true).appendTo('#other_information_table thead');
    $('#other_information_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#other_information_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
