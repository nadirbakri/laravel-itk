	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ url('personel') }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_city_code.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="city_code_table" class="table hover">
				<thead>
					<tr>
						<th>City Code</th>
						<th>City Name</th>
...
<script type="text/javascript">
  $(document).ready(function() {
  	$('#city_code_table thead tr').clone(true).appendTo('#city_code_table thead');
    $('#city_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#city_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personel/city_code/table') }}",
    ...
    $('#city_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personel/city_code/detail_data') }}", { 'city_code' : data.city_code }, "GET", "iframe_dashboard");
    });
