...
<body>
	<div class="div-personel">
		...
		<div class="div-table">
			<table id="type_of_course_code_table" class="table hover">
				<thead>
					<tr>
...
<script type="text/javascript">
  $(document).ready(function() {
  	$('#type_of_course_code_table thead tr').clone(true).appendTo('#type_of_course_code_table thead');
    $('#type_of_course_code_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#type_of_course_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personel/type_of_course_code/table') }}",
        ...
        columns: [
            {data: 'course_type_code', name: 'course_type_code'},
            {data: 'course_type_name', name: 'course_type_name'},
            {data: 'status', name: 'status'}
        ]
    });

    $('#type_of_course_code_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personel/type_of_course_code/detail_data') }}", { 'course_type_code' : data.course_type_code }, "GET", "iframe_dashboard");
    });
