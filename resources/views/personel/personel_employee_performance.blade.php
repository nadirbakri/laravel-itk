	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ url('personel') }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_performance.list') }}</span>
			</a>
		</div>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('#performance_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#performance_table').DataTable({
        ...
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personel/employee_performance/table') }}",
    ...
    $('#performance_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personel/employee_performance/detail_data') }}", { 'employee_no' : data.employee_no, 'employee_name' : data.employee_name }, "GET", "iframe_dashboard");
    });
