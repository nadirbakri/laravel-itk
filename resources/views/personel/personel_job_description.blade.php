...
<html>
<head>
	<title>{{ __('personel_job_description.judul') }}</title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="{{ url('/personnel') }}" target="iframe_dashboard">
				...
				<span class="title-text">{{ __('personel_job_description.list') }}</span>
			</a>
		</div>

		<div class="div-table">
			<table id="job_description_table" class="table hover">
				<thead>
					<tr>
						<th>Position Code</th>
...
<script type="text/javascript">
  $(document).ready(function() {
  	$('#job_description_table thead tr').clone(true).appendTo('#job_description_table thead');
    $('#job_description_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#job_description_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
