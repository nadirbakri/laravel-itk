...
<head>
    <title>{{ __('personel_import_employee_family.judul') }}</title>
    ...
    <div class="div-personel">
        <div class="div-title">
            <a href="{{ route('personnel', ['moduleID' => 'PE']) }}" target="iframe_dashboard" id="toolbar-prev-page">
                ...
                <span class="title-text">{{ __('personel_import_employee_family.list') }}</span>
            </a>
        </div>
    </div>
    <div class="div-form">
        <form id="import_employee_family_form" method="post" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-6">
                    <div class="form-group">
                        <label for="import_export">{{ __('personel_import_employee_family.label_import_export') }}</label>
                        ...
                        <small id="import_export_help" class="text-muted">
                            {{ __('personel_import_employee_family.help_import_export') }}
                        </small>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-3">
                    <button type="button" class="btn btn-primary" name="btn-import" id="btn-import"
                        style="width: 100%;">
                        {{ __('personel_import_employee_family.btn-import') }}
                    </button>
                </div>
                <div class="col-3">
                    <button type="button" class="btn btn-primary" name="btn-download-template" id="btn-download-template"
                        style="width: 100%;">
                        {{ __('personel_import_employee_family.btn-download-template') }}
                    </button>
                </div>
            </div>
        </form>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="div-title-notification">
                        ...
                        <span class="title-text-notification">{{ __('personel_import_employee_family.alert_success') }}</span>
                    </div>
                    <div class="div-title-notification">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
            ...
            $('<div></div>').appendTo('body')
                ...
                buttons: {
                    Yes: function() {
                        ...
                        if ($('#import_employee_family_form').valid()) {
                            $.ajaxSetup({
                                headers: {
                            ...
                            var myform = document.getElementById("import_employee_family_form");
                            var formdata = new FormData(myform);

                            $.ajax({
                                url: "{{ url('personnel/import_employee_family/import') }}",
                                type: "POST",
                                processData: false,
                                contentType: false,
                                data: formdata,
                                ...
                                success: function (response) {
                                    ...
                                    if (response[0].status == "true") {
                                        ...
                                        setTimeout(function () {
                                            window.location =
                                                "{{ url('personnel/import_employee_family') }}";
                                        }, 3000);
                                    } else {
        ...
        $("#btn-download-template").click(function () {
            ...
            if ($('#import_employee_family_form').valid()) {
                ...
                $.ajax({
                    ...
                    url: "{{ url('personnel/import_employee_family/download_template') }}",
                    type: "POST",
                    processData: false,
                    contentType: false,
                    data: formdata,
        ...
        $("#import_employee_family_form").validate({
            rules: {
                import_export: {
                    extension: "xls|xlsx",
                },
            ...
            messages: {
                import_export: {
                    extension: "{{ __('personel_import_employee_family.import_export_extension') }}",
                },
            },
