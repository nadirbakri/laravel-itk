...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('#work_detail_table thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#work_detail_table').DataTable({
        ...
        serverSide: true,
        orderCellsTop: true,
        ajax: "{{ url('personel/work_detail/table') }}",
    ...
    $('#work_detail_table tbody').on('click', 'tr', function () {
    	var data = table.row(this).data();
    	$.redirect("{{ url('personel/work_detail/detail_data') }}", { 'employee_no' : data.employee_no, 'employee_name' : data.employee_name }, "GET", "iframe_dashboard");
    });
