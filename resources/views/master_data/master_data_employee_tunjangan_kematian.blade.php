...
<html>
<head>
    <title>{{ __('data_employee_group.judul_employee_group_tunj_kematian') }}</title>
    ...
    <div class="div-form">
        <div class="div-title">
            <a href="{{ route('master_data', ['moduleID' => 'MOB']) }}" target="iframe_dashboard">
                ...
                <span class="title-text">{{ __('data_employee_group.judul_employee_group_tunj_kematian') }}</span>
            </a>
        </div>
        <form id="md_employee_group" method="post">
            ...
            <div class="card-header">
                <h5>{{ __('data_employee_group.judul_employee_group_tunj_kematian') }}</h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-2">
...
<script>
   ...
   $('#btn-list').click(()=> {
        ...
        table2 = $('#example').DataTable({
            ...
            orderCellsTop: true,
            ajax: {
                url : "{{ url('master_data_tunj_kematian/list/table') }}"
            },
    ...
    $('#group_code').blur(function () {
        if($(this).val() != null && $(this).val() != ''){
            $.ajax({
                type: 'GET',
                url: "{{ url('/master_data/tunj_kematian/get') }}",
                data: {
                    'groupCode': $(this).val()
                }
            }).then(function (data) {
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                if (response.status == "true"){
                                    ...
                                    setTimeout(function () {
                                    window.location =
                                        "{{ url('master_data/employee_group_tunj_kematian') }}";
                                    }, 3000);
                                } else {
                                $("#btn-save").prop("disabled", false);
                                $("#btn-save").html(
