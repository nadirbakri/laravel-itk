...
<html>
<head>
    <title>{{ __('data_employee_master.judul_employee_group_member') }}</title>
        ...
        <form id="md_employee_group" method="post">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-2">
                        <div class="form-group">
                            <label for="exampletwo form-check-label"><b>{{ __('data_employee_group_detail.judul2') }}</b></label>
                        </div>
                    </div>
                </div>
   ...
   $('#btn-list').click(()=> {
    ...
    $('#group_code').blur(function () {
        if($(this).val() != null && $(this).val() != ''){
            $.ajax({
                type: 'GET',
                url: "{{ url('/master_data/detail/get') }}",
                data: {
                    'groupCode': $(this).val()
                }
            }).then(function (data) {
    ...
    function load_data_approval_detail(){
        table4 = $('#examplethree').DataTable({
            ...
            orderCellsTop: true,
            ajax: {
                url : "{{ url('master_data/employee_group_detail/get') }}"             
            },
            error: function(jqXHR, ajaxOptions, thrownError) {
                alert(thrownError + "\r\n" + jqXHR.statusText + "\r\n" + jqXHR.responseText + "\r\n" + ajaxOptions.responseText);
            },
