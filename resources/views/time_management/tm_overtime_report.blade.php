    ...
    <div class="div-time_management">
        <div class="div-title">
            <a href="{{ route('time_management', ['moduleID' => 'TM']) }}" target="iframe_dashboard">
                ...
                <span class="title-text">{{ __('tm_overtime_report.list') }}</span>
            </a>
        </div>
        <div class="div-form">
            <form id="overtime_report_form" method="post">
                @csrf    
                <div class="row">
                    <div class="col-6">
                        <div class="form-group">
                            <label for="period">{{ __('tm_overtime_report.label_period') }}</label>
                            <span class="required">*</span>
                            <div class='input-group'>
                                <input type="text" class="form-control" id="period" name="period"
                                    placeholder="{{ __('tm_overtime_report.label_period') }}">
                ...
                <div class="row">
                    <div class="col-3">
                        <button type="submit" class="btn btn-primary" name="btn-print" id="btn-print"
                            style="width: 100%;">
                            <i class="fa fa-print"></i> {{ __('tm_overtime_report.btn_print') }}
                        </button>
                    </div>
                </div>
            </form>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        let pickerPeriod = $('.input-group input').flatpickr({
            altInput: true,
            allowInput: true,
            altFormat: "d-M-Y",
            dateFormat: "Y-m-d",
        ...
        $("#btn-print").click(function () {
            ...
            $("#overtime_report_form").submit();
        });

        if ($("#overtime_report_form").length > 0) {
            $("#overtime_report_form").validate({
                rules: {
                    period: {
                        required: true,
                    },
                ...
                messages: {
                    period: {
                        required: "{{ __('tm_overtime_report.period_required') }}",
                    },
                },
                ...
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        ...
                        url: "{{ url('time_management/overtime_report/print') }}",
                        type: "POST",
                        data: $('#overtime_report_form').serialize(),
