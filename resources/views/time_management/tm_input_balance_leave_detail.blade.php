    ...
    <div class="div-time_management">
        <div class="div-title">
            <a href="{{ url()->previous() }}" target="iframe_dashboard">
                ...
                <span class="title-text">{{ __('tm_input_balance_leave.list_detail') }}</span>
            </a>
        </div>
        <div class="div-form">
            <form id="input_balance_leave_form" method="post">
                ...
                <div class="row">
                        ...
                        <div class="form-group">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control select2" id="employee_no" name="employee_no"
                                    placeholder= "{{ __('tm_input_balance_leave.label_employee_no') }}" readonly>
                            </div>
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            <div class="form-group">
                                <label
                                    for="leave_code">{{ __('tm_input_balance_leave.label_leave_code') }}</label>
                                <input type="text" class="form-control" id="leave_code"
                                    name="leave_code"
                                    placeholder="{{ __('tm_input_balance_leave.label_leave_code') }}" readonly>
                                {{-- <input type="hidden" class="form-control" id="leave_name" name="leave_name"> --}}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            <label
                                for="expired_date">{{ __('tm_input_balance_leave.label_expired_date') }}</label>
                            <div class='input-group'>
                                <input type="text" class="form-control" id="expired_date" name="expired_date"
                                    placeholder="{{ __('tm_input_balance_leave.label_expired_date') }}">
                ...
                <div class="row">
                    ...
                    <div class="col-3">
                        <a class="btn btn-outline-primary" href="{{ url('time_management/input_balance_leave') }}" target="iframe_dashboard"
                            name="btn-cancel" id="btn-cancel" style="width: 100%;">
                            <i class="fa fa-times-circle"></i> {{ __('tm_input_balance_leave.btn_cancel') }}
                        </a>
                    </div>
                </div>
            </form>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="div-title-notification">
                        ...
                        <span class="title-text-notification">{{ __('tm_input_balance_leave.alert_success') }}</span>
                    </div>
                    <div class="div-title-notification">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#leave_code').val("{{ isset($data[0]->leaveCode) ? $data[0]->leaveCode : '' }}");
        $('#leave_balance').val("{{ isset($data[0]->leaveBalance) ? $data[0]->leaveBalance : '' }}");
        $('#leave_balance_before').val("{{ isset($data[0]->leaveBalanceBefore) ? $data[0]->leaveBalanceBefore : '' }}");
        // $('#leave_name').val("{{ isset($data[0]->leaveName) ? $data[0]->leaveName : '' }}");

        pickerExpiredDate.setDate("{{ isset($data[0]->leaveBalanceBeforeExpiredDate) ? $data[0]->leaveBalanceBeforeExpiredDate : '' }}");
        ...
        if ($("#input_balance_leave_form").length > 0) {
            $("#input_balance_leave_form").validate({
                ...
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        url: "{{ url('time_management/input_balance_leave/proses') }}",
                        type: "POST",
                        ...
                        success: function (response) {
                            if (response.status == "true") {
                                ...
                                setTimeout(function () {
                                    window.location =
                                        "{{ url('time_management/input_balance_leave') }}";
                                }, 3000);
                            } else {
                                $("#btn-save").prop("disabled", false);
                                $("#btn-save").html(
