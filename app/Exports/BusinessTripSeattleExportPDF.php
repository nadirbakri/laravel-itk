...
class BusinessTripSeattleExportPDF implements FromView, ShouldAutoSize
{
    ...
    public function view(): View
    {
        ...
        if($arrResult->dataListSet == null){
            $pdf = PDF::loadView('export.exp_businesstripsettlementpdf_list', ['data' => []])->setPaper('a4', 'landscape')->setOptions(['isPhpEnabled'=> true]);
            return $pdf->stream('Business Trip Settlement.pdf');
        }else{
            $pdf = PDF::loadView('export.exp_businesstrippdsettlementf_list', ['data' => $arrResult->dataListSet])->setPaper('a4', 'landscape')->setOptions(['isPhpEnabled'=> true]);
            return $pdf->stream('Business Trip Settlement.pdf');
        }
    }
}
