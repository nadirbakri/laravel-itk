...
class PositionDataImport implements ToCollection, SkipsEmptyRows, WithStartRow
{
    private $arrResult = [];
        ...
        try {
            ...
            foreach ($rows as $row) {
                $param[] = [
                    ...
                    "positionCode" => (isset($row[0])) ? trim($row[0]) : null,
                    "positionName" => (isset($row[1])) ? $row[1] : null,
                    "supervisorPositionCode" => (isset($row[2])) ? trim($row[2]) : null,
                    "changedNo" => 0,
