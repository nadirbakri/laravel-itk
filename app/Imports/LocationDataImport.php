...
class LocationDataImport implements ToCollection, SkipsEmptyRows, WithStartRow
{
    private $arrResult = [];
        ...
        try {
            ...
            foreach ($rows as $row) {
                $param[] = [
                    ...
                    "locationCode" => (isset($row[0])) ? (string) trim($row[0]) : null,
                    "locationName" => (isset($row[1])) ? (string) $row[1] : null,
                    "bpjsLocationCode" => (isset($row[2])) ? (string) trim($row[2]) : null,
                    "changedNo" => 0,
