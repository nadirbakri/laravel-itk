...
class LevelDataImport implements ToCollection, SkipsEmptyRows, WithStartRow
{
    private $arrResult = [];
        ...
        try {
            ...
            foreach ($rows as $row) {
                $param[] = [
                    ...
                    "levelType" => (isset($row[0])) ? (string) trim($row[0]) : null,
                    "levelCode" => (isset($row[1])) ? (string) trim($row[1]) : null,
