    ...
    public function importUpdateAbsenteeismData(Request $request)
    {
        try{
            ...
            if ($extension === 'xlsx') {
                ...
                Excel::import($import, $file->getRealPath(), null, \Maatwebsite\Excel\Excel::XLS);
            } else {
                return [(object)[
                    'status' => false,
                    'message' => 'File format not supported. Please upload .xls or .xlsx file.',
                ]];
            }

            $results = $import->getArrResult();

            if (empty($results)) {
                return [(object)[
                    'status' => false,
                    'message' => "The Uploaded File Doesn't Match The Template"
                ]];
            }
    ...
    public function prosesShiftMasterCodeTM(Request $request)
    {
        ...
        try {
            ...
            $param = [
                ...
                'flagCrossDay' => isset($request->check_cross_day) ? (bool) $request->check_cross_day : false,
                'crossHourTolerance' => $request->cross_hour_tolerance,
