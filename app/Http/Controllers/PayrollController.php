...
class PayrollController extends Controller
{
    ...
    public function pageOvertimeDetailReport()
    {
        try {
            $client = new Client([
                'verify' => false,
        ...
        return view ('payroll.py_overtime_detail_report', ['data' => $arrResult->dataListSet]);
    }
...
public function dataDetailReportFormatPY(Request $request)
{
        ...
        try {
            ...
            $response = $client->put(env('API_URL') . '/payroll/UpdateSalaryCalculation',
                ['body' => json_encode(
                    [
                        "companyCode" => Session::get('companyCode'),
                        "employeeNoFrom" => $request->employee_no_from_salary,
                        "employeeNoTo" => $request->employee_no_to_salary,
                        "periodMonth" => (int) $request->process_period_salary_month_hidden,
                        "periodYear" => (int) $request->process_period_salary_year_hidden,
                        "loanPaymentProcess" => isset($request->loan_payment_process_salary) ? (bool) $request->loan_payment_process_salary : false,
                        "retroactiveProcess" => isset($request->retroactive_process_salary) ? (bool) $request->retroactive_process_salary : false,
        ...
        try {
            ...
            $response = $client->post(env('API_URL') . '/payroll/insertTmFixedComponent',
                ['body' => json_encode(
                    [
                        "companyCode" => Session::get('companyCode'),
                        "periodMonth" => (int) $request->process_period_absenteeism_month_hidden,
                        "periodYear" => (int) $request->process_period_absenteeism_year_hidden,
                        "employeeNoFrom" => $request->employee_no_from_absenteeism,
                        "employeeNoTo" => $request->employee_no_to_absenteeism,
    ...
    public function prosesJournalProcessPY(Request $request)
    {
        ...
        try {
            ...
            //         "groupAuthorizeFrom" => "string",
            //         "groupAuthorizeTo" => "string",
            //         "languageCode" => App::getLocale(),
            //         "sessionID" => 0,
            //         "sessionUserID" => Session::get('userID'),
            //         "logActionUserID" => Session::get('userID'),
            ...
            $response = $client->post(env('API_URL') . '/payroll/journalprocess',
                ['body' => json_encode(
                    [
                        "companyCode" => Session::get('companyCode'),
                        "journalPeriod" => $request->period_year . "-" . $request->period_month . "-01",
            ...
            $response = $client->post(env('API_URL') . '/payroll/PrTaxCalculationProcess',
                ['body' => json_encode(
                    [
                        ...
                        "employeeNoFrom" => $request->employee_no_from,
                        "employeeNoTo" => $request->employee_no_to,
                        "groupAuthorizeFrom" => intval($request->group_authorized_code_from),
                        "groupAuthorizeTo" => intval($request->group_authorized_code_to),
                        "languageCode" => App::getLocale(),
                        "sessionID" => 0,
                        "sessionUserID" => Session::get('userID'),
                        "logActionUserID" => Session::get('userID'),
    ...
    public function getPaymentSlipPayroll(Request $request){
        ...
        if($data->companyCode == 'DAA'){
            ...
            $viewNamePortrait = 'payroll.py_export_payment_slip_portrait_mca';
    ...
    public function printPeriodicalReportPayroll(Request $request){
            ...
            if(!empty($dataLevel)){
                foreach($dataLevel as $key => $value){
                    ...
                    if (isset($request->grouping_level)) {
                        if(!empty($dataLevel[$key]) && isset($flag_level) && (bool) $flag_level){
                            foreach($dataLevel[$key] as $value2){
                                $data_level_detail[] = [
                                    'levelCode' => $value2
                                ];
        ...
        $customPaper = array(0,0,792.00,1224.00);
        if($arrResult->dataListSet == null){
            $pdf = PDF::loadView('payroll.py_export_periodical_report', ['param' => $param, 'grandTotal' => [], 'data' => [], 'data_company' => $arrCompany->dataListSet, 'data_period' => $request->period, 'grand_total' => isset($request->grand_total) ? (bool) $request->grand_total : false, 'report_name' => $request->report_name_detail, 'group_name' => $request->select_grouping_detail, 'print_signature' => isset($request->print_signature) ? (bool) $request->print_signature : false, 'level1' => $dataLevel[0], 'flag_grand_total' => $flag_grand_total ?? [], 'flag_page_break' => $flag_page_break ?? [], 'company' => Session::get('companyCode')])->setPaper($customPaper, 'landscape')->setOptions(['defaultFont' => 'arial']);
    ...
    public function printPeriodicalReportPayrollExcel(Request $request){
        ...
        return Excel::download(new PeriodicalReportExport(
            ...
            isset($request->grouping_level) ? (bool) $request->grouping_level : false,
            $request->flagLevel ?? [],
