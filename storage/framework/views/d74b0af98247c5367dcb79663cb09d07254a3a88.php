	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="javascript:void(0);" onclick="goBackWithModuleID()" target="iframe_dashboard">
				...
				<span class="title-text"><?php echo e(__('personel_position.list')); ?></span>
			</a>
		</div>
			...
			<table id="position_table" class="table hover">
				<thead>
					<tr>
                        ...
                        <!-- <th>Supervisor Position Name</th> -->
						<th>Status</th>
					</tr>
				</thead>
			</table>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('#position_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#position_table').DataTable({
        ...
        serverSide: true,
        orderCellsTop: true,
        ajax: "<?php echo e(url('personnel/position/table')); ?>",
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("<?php echo e(url('personnel/position/detail_data')); ?>", { 'positionCode' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("<?php echo e(url('personnel/position/detail_data')); ?>", { 'positionCode' : data[0].positionCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "<?php echo e(url('personnel/position/status')); ?>",
                type: "GET",
                ...
                success: function(response) {
                    ...
                    var oTable = $('#position_table').dataTable();
                    oTable.fnDraw(false);
                },
    ...
    $('#position_table tbody').on('click', 'tr td:not(:first-child)', function () {
        ...
        $.redirect("<?php echo e(url('personnel/position/detail_data')); ?>", { 'positionCode' : data.positionCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
    
  });
</script>

</html><?php /**PATH C:\xampp\htdocs\laravel_project\resources\views/personel/personel_position.blade.php ENDPATH**/ ?>