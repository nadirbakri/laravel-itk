...
<head>
	...
	<link rel="stylesheet" href="<?php echo e(asset('css/medical_detail.css')); ?>">
	<style type="text/css">
		.div-medical {
			max-width: 97%;
			margin: auto;
	...
	<div class="div-medical">
		...
		<div class="div-title">
			<a href="javascript:void(0);" onclick="goBackWithModuleID()" target="iframe_dashboard">
				...
				<span class="title-text"><?php echo e(__('md_claim_code.list')); ?></span>
			</a>
		</div>

		<div class="div-table">
			<table id="claim_code_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Claim Code</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#claim_code_table thead tr').clone(true).appendTo('#claim_code_table thead');
    $('#claim_code_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#claim_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "<?php echo e(url('medical/claim_code/table')); ?>",
    ...
    function load_data_claim_code() {
        table = $('#claim_code_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "<?php echo e(url('medical/claim_code/table')); ?>",
    ...
    $('#claim_code_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#claim_code_table').on('click', 'tr td:first-child', function(e){
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("<?php echo e(url('medical/claim_code/detail_data')); ?>", { 'claimCode' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("<?php echo e(url('medical/claim_code/detail_data')); ?>", { 'claimCode' : data[0].claimCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "<?php echo e(url('medical/claim_code/status')); ?>",
                type: "GET",
                data: { 'claimCode' : data[0].claimCode, 'func' : 'A' },
                success: function(response) {
                    ...
                    var oTable = $('#claim_code_table').dataTable();
                    oTable.fnDraw(false);
                },
    ...
    $("#toolbar-deactive").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "<?php echo e(url('medical/claim_code/status')); ?>",
                type: "GET",
                data: { 'claimCode' : data[0].claimCode, 'func' : 'D' },
                success: function(response) {
                    if(response.status == "true"){
                        $('#notification_success').modal('show');
                        $('#message-notification-success').html(response.message);
    ...
    $('#claim_code_table tbody').on('click', 'tr td:not(:first-child)', function () {
    	var data = table.row(this).data();
    	$.redirect("<?php echo e(url('medical/claim_code/detail_data')); ?>", { 'claimCode' : data.claimCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
    
  });
</script>

</html><?php /**PATH C:\xampp\htdocs\laravel_project\resources\views/medical/md_claim_code.blade.php ENDPATH**/ ?>