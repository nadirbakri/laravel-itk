...
<body>
	<div class="div-personel">
		...
		<div class="div-table">
			<table id="competency_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Employee No</th>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $.ajax({
  	...
  	$('#competency_table thead tr').clone(true).appendTo('#competency_table thead');
    $('#competency_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#competency_table').DataTable({
        ...
        serverSide: true,
        orderCellsTop: true,
        ajax: "<?php echo e(url('personnel/competency/table')); ?>",
    ...
    function load_data_competency() {
        table = $('#competency_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "<?php echo e(url('personnel/competency/table')); ?>",
    ...
    $('#notification_success').on('hide.bs.modal', function () {
        window.location = "<?php echo e(url('personnel/competency')); ?>";
    });
    ...
    $("#toolbar-edit").on('click', function() {
        ...
        if(data.count() > 0){
            $.redirect("<?php echo e(url('personnel/competency/detail_data')); ?>", { 'employeeNo' : data[0].employeeNo }, "GET", "iframe_dashboard");
        }else{
            $('#notification_error').modal('show');
    ...
    $('#competency_table tbody').on('click', 'tr td:not(:first-child)', function () {
    	...
    	$.redirect("<?php echo e(url('personnel/competency/detail_data')); ?>", { 'employeeNo' : data.employeeNo }, "GET", "iframe_dashboard");
    });
    
  });
</script>

</html><?php /**PATH C:\xampp\htdocs\laravel_project\resources\views/personel/personel_competency.blade.php ENDPATH**/ ?>