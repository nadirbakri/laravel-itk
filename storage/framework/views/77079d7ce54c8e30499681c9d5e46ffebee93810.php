	...
	<div class="div-utilities">
		...
		<div class="div-title">
			<a href="<?php echo e(url('/utilities')); ?>" target="iframe_dashboard">
				...
				<span class="title-text"><?php echo e(__('utilities_company_master.list')); ?></span>
			</a>
		</div>

		<div class="div-table">
			<table id="company_master_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Company Code</th>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#company_master_table thead tr').clone(true).appendTo('#company_master_table thead');
    $('#company_master_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#company_master_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "<?php echo e(url('utilities/company/table')); ?>",
        ...
        columns: [
            ...
            {data: 'holdingCompany', name: 'holdingCompany'},
    ...
    function load_data_position() {
        table = $('#company_master_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "<?php echo e(url('utilities/company/table')); ?>",
    ...
    $('#company_master_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#company_master_table').on('click', 'tr td:first-child', function(e){
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("<?php echo e(url('utilities/company/detail_data')); ?>", { 'companyCode' : null, 'companyName' : null, 'npwpNo' : null, 'holdingCompany' : null, 'locationCode' : null, 'address' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("<?php echo e(url('utilities/company/detail_data')); ?>", { 'companyCode' : data[0].companyCode, 'companyName' : data[0].companyName, 'npwpNo' : data[0].npwpNo, 'holdingCompany' : data[0].holdingCompany, 'locationCode' : data[0].locationCode, 'address' : data[0].address, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "<?php echo e(url('utilities/company/status')); ?>",
                type: "GET",
                data: { 'companyCode' : data[0].companyCode, 'companyName' : data[0].companyName, 'npwpNo' : data[0].npwpNo, 'holdingCompany' : data[0].holdingCompany, 'locationCode' : data[0].locationCode, 'address' : data[0].address, 'func' : 'A' },
                success: function(response) {
                    ...
                    var oTable = $('#company_master_table').dataTable();
                    oTable.fnDraw(false);
                },
    ...
    $("#toolbar-deactive").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "<?php echo e(url('utilities/company/status')); ?>",
                type: "GET",
                data: { 'companyCode' : data[0].companyCode, 'companyName' : data[0].companyName, 'npwpNo' : data[0].npwpNo, 'holdingCompany' : data[0].holdingCompany, 'locationCode' : data[0].locationCode, 'address' : data[0].address, 'func' : 'D' },
                success: function(response) {
                    if(response.status == "true"){
                        $('#notification_success').modal('show');
                        $('#message-notification-success').html(response.message);
    ...
    $('#company_master_table tbody').on('click', 'tr td:not(:first-child)', function () {
        var data = table.row(this).data();
        $.redirect("<?php echo e(url('utilities/company/detail_data')); ?>", { 'companyCode' : data.companyCode, 'companyName' : data.companyName, 'npwpNo' : data.npwpNo, 'holdingCompany' : data.holdingCompany, 'locationCode' : data.locationCode, 'address' : data.address, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
    
  });
</script>

</html><?php /**PATH C:\xampp\htdocs\laravel_project\resources\views/utilities/utilities_company_master.blade.php ENDPATH**/ ?>