...
<html>
<head>
	<title><?php echo e(__('personel_company_bank.judul')); ?></title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="javascript:void(0);" onclick="goBackWithModuleID()" target="iframe_dashboard">
				...
				<span class="title-text"><?php echo e(__('personel_company_bank.list')); ?></span>
			</a>
		</div>

		<div class="div-table">
			<table id="company_bank_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Bank Code</th>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="div-title-notification">
                        ...
                        <span class="title-text-notification"><?php echo e(__('personel_company_bank.alert_success')); ?></span>
                    </div>
                    <div class="div-title-notification">
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#company_bank_table thead tr').clone(true).appendTo('#company_bank_table thead');
    $('#company_bank_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#company_bank_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: "<?php echo e(url('personnel/company_bank/table')); ?>",
    ...
    function load_data_company_bank() {
        table = $('#company_bank_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "<?php echo e(url('personnel/company_bank/table')); ?>",
    ...
    $('#company_bank_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#company_bank_table').on('click', 'tr td:first-child', function(e){
        $(this).parent().find('input[type="checkbox"]').trigger('click');
    });

    $('#notification_success').on('hide.bs.modal', function () {
        window.location = "<?php echo e(url('personnel/company_bank')); ?>";
    })

    $("#toolbar-new").on('click', function() {
        $.redirect("<?php echo e(url('personnel/company_bank/detail_data')); ?>", { 'bankCode' : null, 'accountNo' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("<?php echo e(url('personnel/company_bank/detail_data')); ?>", { 'bankCode' : data[0].bankCode, 'accountNo' : data[0].accountNo, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "<?php echo e(url('personnel/company_bank/status')); ?>",
                type: "GET",
                data: { 'bankCode' : data[0].bankCode, 'accountNo' : data[0].accountNo, 'func' : 'A' },
                success: function(response) {
                    ...
                    var oTable = $('#company_bank_table').dataTable();
                    oTable.fnDraw(false);
                },
    ...
    $("#toolbar-deactive").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "<?php echo e(url('personnel/company_bank/status')); ?>",
                type: "GET",
                data: { 'bankCode' : data[0].bankCode, 'accountNo' : data[0].accountNo, 'func' : 'D' },
                success: function(response) {
                    if(response.status == "true"){
                        $('#notification_success').modal('show');
                        $('#message-notification-success').html(response.message);
    ...
    $('#company_bank_table tbody').on('click', 'tr td:not(:first-child)', function () {
    	var data = table.row(this).data();
    	$.redirect("<?php echo e(url('personnel/company_bank/detail_data')); ?>", { 'bankCode' : data.bankCode, 'accountNo' : data.accountNo, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
    
  });
</script>

</html><?php /**PATH C:\xampp\htdocs\laravel_project\resources\views/personel/personel_company_bank.blade.php ENDPATH**/ ?>