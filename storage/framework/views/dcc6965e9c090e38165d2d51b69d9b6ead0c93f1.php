...
<html>
<head>
	<title><?php echo e(__('personel_location_code.judul')); ?></title>
	...
	<div class="div-personel">
		...
		<div class="div-title">
			<a href="javascript:void(0);" onclick="goBackWithModuleID()" target="iframe_dashboard">
				...
				<span class="title-text"><?php echo e(__('personel_location_code.list')); ?></span>
			</a>
		</div>

		<div class="div-table">
			<table id="location_code_table" class="table hover">
				<thead>
					<tr>
                        <th></th>
						<th>Location Code</th>
						<th>Location Name</th>
                        <th>BPJS Location Code</th>
						<th>Status</th>
					</tr>
				</thead>
			</table>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('.div-navbar a.disabled').attr('onclick', 'return false;');
  	...
  	$('#location_code_table thead tr').clone(true).appendTo('#location_code_table thead');
    $('#location_code_table thead tr:eq(1) th:not(:first-child)').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#location_code_table').DataTable({
        processing: true,
        serverSide: true,
        orderCellsTop: true,
        ajax: {
            url: "<?php echo e(url('personnel/location/table')); ?>",
            cache: false
        },
        ...
        columns: [
            ...
            {data: 'bpjsLocationCode', name: 'bpjsLocationCode'},
    ...
    function load_data_location_code() {
        table = $('#location_code_table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: {
    ...
    $('#location_code_table tbody').on('click', 'input[type="checkbox"]', function(e){
    ...
    $('#location_code_table').on('click', 'tr td:first-child', function(e){
    ...
    $("#toolbar-new").on('click', function() {
        $.redirect("<?php echo e(url('personnel/location/detail_data')); ?>", { 'locationCode' : null, 'func' : 'new' }, "GET", "iframe_dashboard");
    });

    $("#toolbar-edit").on('click', function() {
        var data = table.rows('.selected').data();
        if(data.count() > 0){
            $.redirect("<?php echo e(url('personnel/location/detail_data')); ?>", { 'locationCode' : data[0].locationCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
        }else{
    ...
    $("#toolbar-active").on('click', function() {
        ...
        if(data.count() > 0){
            $.ajax({
                url: "<?php echo e(url('personnel/location/status')); ?>",
                type: "GET",
                ...
                success: function(response) {
                    ...
                    var oTable = $('#location_code_table').dataTable();
                    oTable.fnDraw(false);
                },
    ...
    $('#location_code_table tbody').on('click', 'tr td:not(:first-child)', function () {
        ...
        $.redirect("<?php echo e(url('personnel/location/detail_data')); ?>", { 'locationCode' : data.locationCode, 'func' : 'edit' }, "GET", "iframe_dashboard");
    });
    
  });
</script>

</html><?php /**PATH C:\xampp\htdocs\laravel_project\resources\views/personel/personel_location_code.blade.php ENDPATH**/ ?>